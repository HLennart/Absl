...
VerifyTypeImplementsAbslHashCorrectly(const Container& values, Eq equals) {
  ...
  for (const auto& value : values) {
    EqClass* c = nullptr;
    for (auto& eqclass ABSL_DLL  : classes) {
      if (absl::visit(EqVisitor<Eq>{equals}, value, eqclass ABSL_DLL [0].value)) {
        c = &eqclass;
        break;
      }
    }
  ...
  for (const auto& c : class ABSL_DLL es) {
    ...
    for (const auto& c2 : class ABSL_DLL es) {
      if (&c == &c2) continue;
